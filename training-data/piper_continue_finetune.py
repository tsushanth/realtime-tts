"""CONTINUATION of piper_full_finetune.py: resumes the full-corpus run from its own final
checkpoint (version_1/last.ckpt, optimizer state included) and trains to 60,000 total
optimizer steps instead of 20,000. Run 1 saw only ~4 epochs (Piper counts generator and
discriminator updates separately, so 20,000 steps = ~10,000 batches); val_mel had
flattened (0.3337 -> 0.3343 -> 0.3343) so the gain may be small - the point is to find out.
`--ckpt_path` (resume) failed for the PRETRAINED lessac checkpoint because of a stale
`sample_bytes` hyperparameter, but our own checkpoint was saved by current code so it should
load; fall back to --model.warmstart_ckpt if it doesn't. New checkpoints land in a new
lightning_logs/version_N directory. Everything else (WAV copy to local disk, progress-logging
patch, .spawn()) is unchanged - see piper_full_finetune.py's docstring for why each exists.
Estimate: ~20,000 more batches at ~1.3 it/s ~= 4.6 h on a T4, roughly $3-4.
...
app = modal.App("tts-piper-full-ft-continue", image=image)

corpus_volume = modal.Volume.from_name("tts-corpus-full")
checkpoint_volume = modal.Volume.from_name("tts-checkpoints")

MAX_STEPS = 60000  # TOTAL optimizer steps (GAN: 2 per batch). Run 1 stopped at 20000 (~4 epochs); +40000 = ~7 more epochs
...
def run_full_piper_finetune():
    ...
    sys.argv = [
        ...
        "--ckpt_path", "/checkpoints/piper_full_ft/lightning_logs/version_1/checkpoints/last.ckpt",  # RESUME our own run
