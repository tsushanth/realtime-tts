"""LICENCE-CLEAN CANDIDATE: exports/benchmarks Piper's published LJ Speech medium checkpoint
(rhasspy/piper-checkpoints en_US/ljspeech/medium, trained from scratch on the public-domain
LJ Speech dataset - no Polly audio, no Lessac base). Same 5 sentences and CPU benchmark as
synthesize_piper_full.py so the two voices can be compared by ear and by RTF.
Outputs: ljspeech_output/. Derived from the script below:
...
image = (
    ...
    .run_commands(
        ...
        "wget -q -O lj.ckpt https://huggingface.co/datasets/rhasspy/piper-checkpoints/resolve/main/en/en_US/ljspeech/medium/lj-med_1000.ckpt && "
        "wget -q -O config.json https://huggingface.co/datasets/rhasspy/piper-checkpoints/resolve/main/en/en_US/ljspeech/medium/config.json"
    )
)

app = modal.App("tts-piper-ljspeech-synth", image=image)
...
@app.function(cpu=4, timeout=1200)
def export_and_synthesize():
    ...
    for i, text in enumerate(TEST_SENTENCES):
        ...
        wav_path = f"/tmp/lj_sample_{i}.wav"
...
def main():
    ...
    out_dir = os.path.join(os.path.dirname(__file__), "ljspeech_output")
