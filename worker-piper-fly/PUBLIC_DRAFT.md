...
| Price | $0.01 / 1K chars | **$0.004 / 1K chars** (decided) |
...
Free tier characters are shared across engines (a Piper character uses one free character).
...
voices. **Decided: no side-by-side ElevenLabs audio.** Publish Piper-only samples instead and
let buyers compare on their own text; the latency and price tables make the case.

**Price per 1M characters (list prices, as of 2026-09):** ElevenLabs Flash $50; ElevenLabs
Multilingual v2 and v3 $100; ReadAloud Kokoro $10; ReadAloud Piper $4.
...
- A dedicated-CPU machine measured 8-16 concurrent calls at ~325-345 ms median before
  degrading, at about 10x the machine cost. **Decided: scale up once there is revenue.**
...
the scripts are in the repo (`benchmarks/`; approved for publishing).

---

## 3. Things that must be true before this is public

1. **Deploy the billing change.** Committed in ReadAloudAI (c453d1f): the backend job weights
   Piper chars 0.4x on the existing Stripe meter. Not deployed - until it is, Piper bills at
   the Kokoro rate. Invoices will show Kokoro-equivalent character quantity.
